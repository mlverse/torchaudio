...
load_cmuarctic_item <- function(
  ...
  utterance_id_utterance = line[0]$strip()$split(" ", 2)[1:N]
  ...
  # Load audio
  waveform_sample_rate = torchaudio$load(file_audio)

  return(
    list(
      waveform = waveform_sample_rate[[1]],
      sample_rate = waveform_sample_rate[[2]],
      utterance = utterance,
...
cmuarctic_dataset <- torch::dataset(
  ...
  initialize = function(
    ...
    if(download) {
      ...
      if(!os.path.isdir(self._path)) {
        if(!os.path.isfile(archive)){
          checksum = .CHECKSUMS.CMUARCTIC[[url]]
    ...
    self$.text = os.path.join(self$.path, self$.folder_text, self$.file_text)

    text = open(self$.text, "r")
    with(text) {
