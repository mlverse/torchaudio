...
First, let's create the train and test subsets. Worth notice that `self$.walker` is a character vector that stores the list of all audio file paths.

```{r}
speechcommand_subset <- torch::dataset(
  "SpeechCommandsSubset",
  inherit = speechcommand_dataset,
  initialize = function(indexes = NULL) {
    super$initialize(root = DATASETS_PATH, url = "speech_commands_v0.01", download = TRUE)
    if(!is.null(indexes))
      self$.walker <- self$.walker[indexes]
  }
)
...
# subsets
train_subset <- speechcommand_subset(indexes = id_train)
test_subset <- speechcommand_subset(indexes = -id_train)
```
...
# Return the position of the word in classes
class_to_index <- function(class) torch::torch_tensor(which(classes == class))

# Return the word corresponding to the index in classes
index_to_class <- function(index) classes[as.numeric(index$to(device = "cpu"))]
...
collate_fn <- function(batch) {
 ...
 # target encoding
 targets <- sapply(targets, class_to_index)
 targets <- torch::torch_stack(targets)
 targets <- targets$squeeze()$to(dtype = torch_long())
...
epochs <- 10
batches <- enumerate(ds_train)
losses <- c()
accs <- c()

for(epoch in seq_len(epochs)) {
  pb <- set_progress_bar(length(batches))  
  ...
  for(batch in batches) {
    ...
    loss$backward()
    optimizer$step()
    # acc <- metrics$acc(targets, as.numeric(predictions))
    loss_r <- round(as.numeric(loss), 5)
    losses <- c(losses, loss_r)
    # accs <- c(accs, acc)
    pb$tick(tokens = list(loss = loss_r, acc = "A"))  
  }
  val_loss <- 3
  val_acc <- 100
  cat(glue("val_loss: {val_loss} - val_acc: {val_acc}\n"))
}
```
