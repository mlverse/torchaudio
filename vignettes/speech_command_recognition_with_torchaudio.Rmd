...
subset_sc <- torch::dataset(
  ...
  initialize = function(subset = NULL) {
    ...
    if(subset == "validation") {
      self$.walker <- load_list("validation_list.txt")
    } else if(subset == "testing") {
      self$.walker <- load_list("testing_list.txt")
...
plot(waveform[1]/(2^15), type = "l", col = "royalblue")
```
...
index_to_label <- function(index) {
 ...
 return(labels[as.numeric(index)])
}
...
train <- function(model, epoch, log_interval) {
  ...
  for(batch_idx in seq_along(batches)) {
    ...
    loss <- nnf_nll_loss(output$squeeze(), target$squeeze()$to(dtype = torch::torch_float()))
...
test <- function(model, epoch) {
  ...
  for(batch_idx in seq_along(batches)) {
    data <- batches[batch_idx][[1]]$to(device = device)
    target <- batches[batch_idx][[2]]$to(device = device)
  ...
  print(glue::glue("
Test Epoch: {epoch}	Accuracy: {correct}/{length(test_loader$dataset)} ({scales::percent(correct / lenght(test_loader$dataset))})"))
}
```
...
```{r}
plot(losses, main = "training loss")
```
...
predict <- function(tensor) {
 ...
 tensor <- tensor$to(device = device) %>%
   transform()$unsqueeze(1L) %>%
   model() %>%
   get_likely_index()$squeeze() %>%
   index_to_label()
 
 return(tensor)
}

sample <- train_set[length(train_set)]
...
```{r}
for i,  <- function(waveform, sample_rate, utterance, *_) { in enumerate(test_set):
