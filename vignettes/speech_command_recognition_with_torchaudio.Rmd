...
```{r, eval = TRUE}
if(!fs::dir_exists("~/datasets")) fs::dir_create("~/datasets")
subset_sc <- torch::dataset(
  "SubsetSC",
  inherit = speechcommand_dataset,
  initialize = function(root = ".", subset = NULL) {
    super$initialize(root, download = TRUE)
...
train_set <- subset_sc("~/datasets", "training")
test_set <- subset_sc("~/datasets", "testing")
...
```{r, echo = FALSE}
sample <- torchaudio:::load_speechcommands_item("~/datasets/SpeechCommands/speech_commands_v0.02/backward/0165e0e8_nohash_0.wav", "./SpeechCommands/speech_commands_v0.02/")
...
```{r labels, cache=FALSE, eval = FALSE}
...
```{r, echo=FALSE}
# torch::torch_save(model, "cache/model.pt")
...
```{r, echo = FALSE}
# saveRDS(losses, "cache/losses.rds", version = 2)
